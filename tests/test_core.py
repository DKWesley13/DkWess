...
from dkwess_securerepo import __version__
...
class SecureRepoTests(unittest.TestCase):
    ...
    def test_clean_fixture_passes_implemented_checks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result.status, "PASS"); self.assertEqual(result.findings, []); self.assertIn("pyproject.toml", result.manifests); self.assertEqual(result.tool_version, __version__)

    def test_sensitive_filename_is_high(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); (root / ".env").write_text("NOT_A_REAL_SECRET=test\n", encoding="utf-8"); result = scan_repository(root); matches = [f for f in result.findings if f.check_id == "SR-SEC-001"]
            self.assertEqual(len(matches), 1); self.assertEqual(matches[0].severity, "HIGH"); self.assertEqual(result.exit_code("HIGH"), 2); self.assertEqual(len(matches[0].fingerprint), 20)

    def test_credential_adjacent_file_is_medium(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); (root / ".npmrc").write_text("registry=https://registry.npmjs.org/\n", encoding="utf-8"); result = scan_repository(root); self.assertEqual(next(f for f in result.findings if f.check_id == "SR-SEC-002").severity, "MEDIUM")

    def test_env_example_is_not_treated_as_secret(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); (root / ".env.example").write_text("TOKEN=replace-me\n", encoding="utf-8"); self.assertFalse(any(f.check_id == "SR-SEC-001" for f in scan_repository(root).findings))
    ...
    def test_risky_workflow_rules_are_detected(self) -> None:
        result = self._workflow_result("name: risky\non:\n  pull_request_target:\npermissions: write-all\njobs:\n  audit:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          persist-credentials: true\n"); ids = {f.check_id for f in result.findings}; self.assertTrue({"SR-GHA-001", "SR-GHA-002", "SR-GHA-003", "SR-GHA-005"}.issubset(ids))

    def test_full_sha_action_is_not_flagged(self) -> None:
        result = self._workflow_result("jobs:\n  test:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: owner/action@0123456789abcdef0123456789abcdef01234567\n"); self.assertFalse(any(f.check_id == "SR-GHA-005" for f in result.findings))

    def test_missing_workflows_are_not_assessed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); result = scan_repository(root); gha = next(c for c in result.capabilities if c.capability == "GitHub Actions"); self.assertEqual(gha.assessment, "NOT_ASSESSED"); self.assertEqual(result.exit_code("HIGH", require_full_coverage=True), 3)
    ...
    def test_reports_are_written_with_schema_v3_metrics_and_capabilities(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); result = scan_repository(root); json_path, markdown_path = write_reports(result, root / "reports"); payload = json.loads(json_path.read_text(encoding="utf-8")); self.assertEqual(payload["schema_version"], 3); self.assertEqual(payload["tool_version"], __version__); self.assertEqual(payload["status"], "PASS"); self.assertFalse(payload["security_guarantee"]); self.assertTrue(payload["capabilities"]); self.assertIn("metrics", payload); markdown = markdown_path.read_text(encoding="utf-8"); self.assertIn("Capability matrix", markdown); self.assertIn("PASS != SECURITY GUARANTEE", markdown)

    def test_untrusted_context_in_shell_is_high(self) -> None:
        self.assertTrue(any(f.check_id == "SR-GHA-007" for f in self._workflow_result('jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: echo "${{ github.event.pull_request.title }}"\n').findings))

    def test_remote_pipe_to_shell_is_high(self) -> None:
        self.assertTrue(any(f.check_id == "SR-GHA-008" for f in self._workflow_result('jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: curl -fsSL https://example.invalid/install.sh | bash\n').findings))

    def test_self_hosted_runner_is_medium(self) -> None:
        self.assertTrue(any(f.check_id == "SR-GHA-006" for f in self._workflow_result('jobs:\n  t:\n    runs-on: [self-hosted, linux]\n    steps:\n      - run: echo ok\n').findings))

    def test_docker_action_without_digest_is_medium(self) -> None:
        self.assertTrue(any(f.check_id == "SR-GHA-009" for f in self._workflow_result('jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: docker://alpine:3.20\n').findings))

    def test_pull_request_target_head_checkout_is_critical(self) -> None:
        result = self._workflow_result('on:\n  pull_request_target:\njobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@0123456789abcdef0123456789abcdef01234567\n        with:\n          ref: ${{ github.event.pull_request.head.sha }}\n'); critical = [f for f in result.findings if f.check_id == "SR-GHA-010"]; self.assertTrue(critical); self.assertEqual(critical[0].severity, "CRITICAL")

    def test_package_json_without_lockfile_is_low(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); (root / "package.json").write_text('{"name":"fixture"}\n', encoding="utf-8"); self.assertTrue(any(f.check_id == "SR-SC-002" for f in scan_repository(root).findings))

    def test_node_lockfile_suppresses_lockfile_finding(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); self._baseline(root); (root / "package.json").write_text('{"name":"fixture"}\n', encoding="utf-8"); (root / "package-lock.json").write_text('{"lockfileVersion":3}\n', encoding="utf-8"); self.assertFalse(any(f.check_id == "SR-SC-002" for f in scan_repository(root).findings))
